...
with source as (
    select *
    from {{ source('nyc_taxi_raw', 'yellow_trips_bronze') }}
),
...
renamed as (
    select
        ...
        -- Conversión desde epoch en nanosegundos
        to_timestamp_ntz(TPEP_PICKUP_DATETIME / 1e9) as pickup_datetime,
        to_timestamp_ntz(TPEP_DROPOFF_DATETIME / 1e9) as dropoff_datetime,

        -- Nuevas columnas solo fecha
        to_date(to_timestamp_ntz(TPEP_PICKUP_DATETIME / 1e9)) as pickup_date,
        to_date(to_timestamp_ntz(TPEP_DROPOFF_DATETIME / 1e9)) as dropoff_date,
